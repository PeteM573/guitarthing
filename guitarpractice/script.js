...
function createCircleOfFifths() {
    ...
    // Define Radii for both rings
    const outerMajorRadius = 45;
    const innerMajorRadius = 35; // Gap between rings
    const outerMinorRadius = innerMajorRadius - 1; // Start minor ring just inside major
    const innerMinorRadius = 25; // Center hole size
    ...
    for (let i = 0; i < numSegments; i++) {
        ...
        const relativeMinorKeyName = MINOR_KEYS[i];
        ...
        segmentMajor.setAttribute('class', 'key-segment-major'); // Specific class
        segmentMajor.setAttribute('data-key', majorKeyName);
        segmentMajor.setAttribute('data-quality', 'Major');
        segmentMajor.setAttribute('data-relative-minor', relativeMinorKeyName); // Store relative key
        segmentMajor.addEventListener('click', () => handleCircleClick(majorKeyName, 'Major', segmentMajor, relativeMinorKeyName));
        circleOfFifthsSVG.appendChild(segmentMajor);

        // --- Create Minor Segment (Inner Ring) ---
        const minorKeyName = MINOR_KEYS[i];
        const relativeMajorKeyName = MAJOR_KEYS[i];
        ...
        segmentMinor.setAttribute('class', 'key-segment-minor'); // Specific class
        segmentMinor.setAttribute('data-key', minorKeyName);
        segmentMinor.setAttribute('data-quality', 'Minor');
        segmentMinor.setAttribute('data-relative-major', relativeMajorKeyName); // Store relative key
        segmentMinor.addEventListener('click', () => handleCircleClick(minorKeyName, 'Minor', segmentMinor, relativeMajorKeyName));
        circleOfFifthsSVG.appendChild(segmentMinor);


        // --- Add Key Labels (Positioned between the rings) ---
        const labelRadius = (innerMajorRadius + outerMinorRadius) / 2; // Place between rings
        ...
        majorText.setAttribute('x', labelX);
        majorText.setAttribute('y', labelY - 2); // Position slightly above center line
        ...
        minorText.setAttribute('y', labelY + 4); // Position slightly below center line
    ...
    // Initial highlight is now handled by initializeApp after this function runs
}
// === END OF ADDING FUNCTION DEFINITION ===
...
function handleCircleClick(keyName, quality, clickedSegment) { // Remove relativeKeyName param
    ...
    // --- Update Visual Selection Directly ---
    ...
    // Add selection ONLY to the clicked segment
    clickedSegment.classList.add('selected');
    console.log(`Highlighted segment for: ${keyName} ${quality}`);
    // --- End of Direct Highlighting ---
...
// === REPLACE highlightCircleSegment (Simplified for Initial Load) ===
function highlightCircleSegment(keyToHighlight, qualityToHighlight) {
    // Remove selection from ALL segments first
    document.querySelectorAll('#circleOfFifths .key-segment-major.selected, #circleOfFifths .key-segment-minor.selected').forEach(el => {
        el.classList.remove('selected');
    });

    // Construct selector based on quality
    const selector = `.key-segment-${qualityToHighlight.toLowerCase()}[data-key="${keyToHighlight}"]`;
    const segment = document.querySelector(selector);

    if (segment) {
        segment.classList.add('selected');
        console.log(`Initial highlight set for: ${keyToHighlight} ${qualityToHighlight}`);
    } else {
        console.warn(`Could not find segment for initial highlight: Key=${keyToHighlight}, Quality=${qualityToHighlight}`);
    }
}
...
function initializeApp() {
    ...
    // --- Initial Highlight (Simplified Call) ---
    // Highlight based on the initial state variables
    ...
    pitchOutputDiv.innerHTML = 'Select Key & Progression, then Start'; // Updated initial message
